# Demos — the engine, seven ways

This folder ships seven silent demo decks (capability reels, not narrated explainers):
...
| **`scene-3d-beauty.html`** / `scene-3d-beauty.mp4` | **Cosmic zoom** — one continuous dive, rendered on the GPU frame-by-frame: a 46,000-star **galaxy** → *flash* → **Earth** (continents, clouds, specular oceans, night-side city lights, atmosphere) → *flash* → a **neon-city flythrough** — glass towers (reflective + baked cube-env) with neon billboards, a wet avenue with **real planar reflections** and defined lane markings, streaming car light-trails, and **depth-of-field bokeh** → the street-level `/storyboard` CTA. ACES tone mapping + bloom + DoF + 2× supersampling. |
