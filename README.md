...
- **A cast & sets layer** — owned procedural **characters** (`data-anim="character"` — a human `person` style + 7 mascots, all with expressions, auto-blink, gaze that tracks elements, gestures, and **lip-sync to the VO**), **device & prop mockups** (phone/laptop/browser/tablet frames, speech bubbles, sticky notes, map pins), and **living environments** (day/dusk/night skies, snow/rain/embers/bubbles particle systems, parallax scenery)
