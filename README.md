...
- **A cast & sets layer** — owned procedural **characters** (`data-anim="character"`: expressions, auto-blink, gaze that tracks elements, gestures, and **lip-sync to the VO**), **device & prop mockups** (phone/laptop/browser/tablet frames, speech bubbles, sticky notes, map pins), and **living environments** (day/dusk/night skies, snow/rain/embers/bubbles particle systems, parallax scenery)
...
- **`examples/showcase/`** — three silent demo decks:
  ...
  - **The cast & sets demo** ([`cast-and-sets.mp4`](examples/showcase/cast-and-sets.mp4)) — the actor layer in action: procedural characters wave, point, talk (lip-sync) and react, staged on device mockups + props (sticky notes, speech bubbles, pins), across living environments (night/day/dusk skies, snow, embers).
