...
- **A cast & sets layer** — owned procedural **characters** (`data-anim="character"` — two human styles: `person` (modern-flat) + `cutout` (construction-paper / South-Park look), plus 7 mascots, all with expressions, auto-blink, gaze that tracks elements, gestures, and **lip-sync to the VO**), **device & prop mockups** (phone/laptop/browser/tablet frames, speech bubbles, sticky notes, map pins), and **living environments** (day/dusk/night skies, snow/rain/embers/bubbles particle systems, parallax scenery). **Bring your own character** two ways: a safe JSON definition (`Storyboard.defineCharacter` / `data-char-src` — and `/storyboard` can generate one from a description or a reference image), or rig your own SVG by tagging parts with `data-sbc` — or do it **visually** with the `rigger.html` tool (drag the eyes & mouth onto your art, copy the code)
- **Real 3D (WebGL)** — `data-anim="scene3d"` runs a **clock-synced three.js scene** (real meshes, lighting, materials, a 3D camera) that scrubs with the VO. Bundled three.js; renders through the headless → MP4 pipeline (software GL, so keep scenes modest)
...
- **`examples/showcase/`** — three silent demo decks:
  ...
  - **The capability gallery** ([`capabilities.mp4`](examples/showcase/capabilities.mp4)) — a ~29-section kitchen-sink catalog that demos *every* family the engine has: entrances, physics, 3D, text & kinetic type, easing/spring, staggers & chains, loops, charts, diagrams, motion paths, annotations, code, the camera rig + multiplane, the cinematic grade, living backgrounds, the full fun pack, baked-in Lottie, exits, and all 21 transitions.
