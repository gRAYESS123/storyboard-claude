...
## Dazzle: fun pack + Lottie on demand (v0.7)

The engine ships ~24 owned, brand-colored, render-safe **fun-pack** effects — confetti, fireworks, checkmarks, spinners, waveforms, badges, rating stars, rocket launch, party popper, floating emoji reactions, and more. Full list + usage in animations.md; when-to-use guidance in animator.md. Reach for these first.

For richer illustrated pieces (mascots, weather, delivery, etc.), use the curated 100-entry index instead of bundling files:
- Browse/search: `python skill/lottie_fetch.py --search "<need>"` (routes to an owned preset when one exists).
- Fetch on use: `python skill/lottie_fetch.py fetch <url> --out <project> --id <id>` — validates the Lottie, saves it locally (offline-safe renders), records the source in assets/lottie/CREDITS.md. **Verify the source license before shipping.**
- Play: `<div class="anim" data-anim="lottie" data-src="assets/lottie/<id>.json">`.
